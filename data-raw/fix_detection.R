...
# Use log-spaced prevalence from 0.01% to 2%
...
# Focus on the interesting range: 0 to just past 95% threshold
# Clip x-axis to show the S-curve clearly
...
p7 <- ggplot2::ggplot(det_df_clip, ggplot2::aes(x = .data$prevalence, y = .data$detection)) +
  ...
  ggplot2::labs(
    ...
    subtitle = paste0("ECDC 5-country design (n = 99,093 weekly sequences) | ",
                      "P(detect \u2265 1 sequence per week)"),
    x = "True variant prevalence in population",
    y = "Weekly detection probability",
    caption = "Based on per-stratum sequencing volumes | Horizontal lines: 50%, 80%, 95% thresholds"
  ) +
...
cat("=== fig7 detection FIXED (focused x-axis) ===\n")
